...
// Only load pool when in DB mode (null when USE_DB=false)
const pool = USE_DB ? require('../utils/db.include') : null;
...
router.get('/available/:carId', async (request, response) => {
    try {
        ...
        if (USE_DB) {
            ...
            // JSON mode: delegate to store repository.
            // getStoresForConfiguration handles both empty and non-empty featureIds:
            //   - empty  -> returns all stores that carry the car
            //   - filled -> returns stores that carry the car AND all selected features
            const stores = await storeRepo.getStoresForConfiguration(carId, featureIds);
            response.json(stores);
        }
    } catch (error) {
